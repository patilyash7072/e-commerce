...
public class MultiThreadingSimulation {
    ...
    public static void start() throws SQLException {
        logger = LoggerFactory.getLogger(MultiThreadingSimulation.class);

        LoginCredential threadOneLogin = new LoginCredential("aamir", "aamir" , UserRole.CUSTOMER);
        LoginCredential threadTwoLogin = new LoginCredential("salmaan", "salmaan" , UserRole.CUSTOMER);

        Thread t1 = new Thread(task(threadOneLogin));
        Thread t2 = new Thread(task(threadTwoLogin));
        t1.start();
        t2.start();
    ...
    static Runnable task(LoginCredential loginCredential) throws SQLException {
        ...
        return () -> {
            ...
            try (Connection connection = DbService.getConnection()) {
                ...
                checkoutService.checkout(cart, PaymentMode.COD, credential);
